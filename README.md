...
- **2026.09.07 코드 정비·재검증:** 아래 성능은 새로 측정한 결과이며 4월 당시 성과와 구분합니다.
...
**팀과 개인의 연결:** 개인별 EDA와 기본 모델 학습 결과를 비교한 뒤, 팀이 선정한 한 구성원의 모델을 공통 파일로 사용해 추가 학습·전처리 분석을 진행했습니다.
...
재검증에서는 **validation macro F1**을 모델 선택 기준으로 사용하고, 정확도와 0 클래스 recall을 함께 확인했습니다.
